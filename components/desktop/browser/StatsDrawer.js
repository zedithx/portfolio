...
export default function StatsDrawer({ skills }) {
    ...
    return (
        <>
            {/* Floating Stats Button - Game Style */}
            <motion.button
                whileHover={prefersReducedMotion ? {} : { scale: 1.1, boxShadow: '0 0 30px rgba(255, 215, 0, 0.8)' }}
                whileTap={prefersReducedMotion ? {} : { scale: 0.9 }}
                onClick={openDrawer}
                className="fixed bottom-6 right-6 z-40 min-h-[64px] min-w-[64px] px-5 py-4 rounded-full shadow-2xl font-bold text-base md:text-lg touch-manipulation flex items-center gap-2 relative overflow-hidden"
                style={{
                    background: 'linear-gradient(135deg, #ff6b6b 0%, #ee5a6f 100%)',
                    border: '3px solid #ffd700',
                    color: '#fff',
                    textShadow: '2px 2px 4px rgba(0, 0, 0, 0.8)',
                    boxShadow: '0 0 25px rgba(255, 107, 107, 0.6), inset 0 0 10px rgba(255, 255, 255, 0.2)'
                }}
            ...
            >
                ...
                <span className="hidden sm:inline relative z-10">⚔️ STATS</span>
                <span className="sm:hidden relative z-10 text-2xl">⚔️</span>
            </motion.button>
                            ...
                            {Object.entries(skills).map(([skillName, skill], index) => {
                                ...
                                const getSkillColor = (name) => {
                                    switch(name) {
                                        ...
                                        case 'Cloud': return { from: '#4facfe', to: '#00f2fe', glow: 'rgba(79, 172, 254, 0.8)' };
